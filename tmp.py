...
def getrtmTask(jsonp):
    ...
    curt = (datetime.datetime.strptime(datetime.datetime.now().strftime("%Y-%m-%d\
    "), ("%Y-%m-%d"))).timestamp()
    jsoncts = (json.loads(jsonp))['tasks']
    taskContl = []
    for task in jsoncts:
        if task['date_due_has_time'] and task['date_due'] > (curt-86400)*1000 \
         and task['date_due'] < curt*1000:
            taskContl.append((datetime.datetime.fromtimestamp(task['date_d\
                ue']/1000)).strftime("%H:%M") + task['name'])
    return(taskContl)
